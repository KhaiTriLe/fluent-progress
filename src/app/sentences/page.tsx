    ...
    <div className="container mx-auto max-w-4xl p-4 md:p-8">
           ...
           <AccordionItem key={topic.id} value={topic.id} className="rounded-lg border bg-card shadow-sm">
             ...
             <AccordionContent className="p-4 pt-0">
               <ul className="space-y-3">
                ...
                <Button variant="outline" size="sm" className="mt-4" onClick={() => handleOpenFormForNewSentence(topic)}><Plus className="mr-2 h-4 w-4" /> Add Sentence</Button>
             </AccordionContent>
           </AccordionItem>
         ))}
       </Accordion>
      ...
      <Dialog open={isFormOpen} onOpenChange={setIsFormOpen}>
        <DialogContent>
          <DialogHeader>
            <DialogTitle>
                {editingTopic && !editingSentence ? `Add Sentence to "${editingTopic.name}"` : 
